        ...
        if ((!event.isCancelled()) && (event.getEntity() instanceof Wolf) && (event.getOwner() instanceof Player)) {
            ...
            if (tamer.hasPermission("wolfpay.tame")) {
                ...
                if (wolves.size() <= allowedwolves) {
                    ...
                    tamer.sendMessage(ChatColor.GREEN.toString()
                            + WolfPay.getMessage("havenow").replaceAll("x", String.valueOf(wolves.size()))
                    ...
                    if (tamer.hasPermission("wolfpay.pay")) {
                        ...
                        if (bank.hasEnough(tamer, WolfPay.price, -1)) {
                            ...
                            tamer.sendMessage(ChatColor.GREEN
                                    + WolfPay.getMessage("successpay").replaceAll("money", bank.getFormattedAmount(tamer, WolfPay.price, -1)));
